...
class BackgammonEnv:
    ...
    def step(self, action, player, movement):
        ...
        #         return self.get_state(), total_reward + 10, True, 'white'
        # elif player == 'black':
        #     if self.count_negative_fields() == 0:
        #         return self.get_state(), total_reward + 10, True, 'black'
    ...
    def _is_valid_move(self, from_pos, to_pos, player):
        ...
        if player == 'white' and self.board[from_pos] < 0:
            return False
        if player == 'black' and self.board[from_pos] > 23:
            return False

        # if player == 'white' and self.board[to_pos] <= -1:
        #   return False
        # if player == 'black' and self.board[to_pos] > 24:
        #    return False
...
# scores = train_agent(env, agent_white, agent_black, num_episodes)
...
print("Starting pre-training for test agent...")
test_agent = Agent(gamma, epsilon, lr, input_size, batch_size, n_actions)
# train_movement(env, test_agent, num_episodes=1000)


# 5.4 Test przeciwników
# not_trained_agent = Agent(gamma=0.99, epsilon=0.1, lr=0.001,
#                       input_size=26, batch_size=32, n_actions=24)
# softmax_agent = Agent(gamma=0.99, epsilon=0.1, lr=0.001,
#                       input_size=26, batch_size=32, n_actions=24, exploration_type='softmax')
# print("Pre-trained test agent vs not trained agent...")
#test_against_different_opponents(env, test_agent)
#
# print("Pre-trained test agent vs not trained softmax agent...")
# test_against_different_opponents(env, softmax_agent)
