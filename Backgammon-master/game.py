...
class Agent:
    def __init__(self, gamma, epsilon, lr, input_size, batch_size, n_actions, max_mem=10000, eps_end=0.01,
                 eps_decay=1e-1, exploration_type='epsilon_greedy', temp=1.0):
    ...
    def choose_action(self, state, dice_roll, player, forbidden_actions=None, trained=False):
        ...
        if trained == 0:
            ...
            return self.choose_action_trained(state, dice_roll, player)
    ...
    def choose_action_trained(self, state, dice_roll, player):
        ...
        mask = T.full((self.n_actions,), float('-inf'), device=eval_net.device)  # Wszystkie akcje -inf
        mask[possible_fields] = 0  # Dozwolone akcje mają wartość 0 (nie zmieniają Q)

        move_dice_pair = T.zeros((1, 2)).to(eval_net.device)

        with T.no_grad():
            q_values = eval_net.forward(state_tensor, move_dice_pair)
            q_values = q_values + mask  # Ustawiamy -inf dla niedozwolonych akcji
        ...
        if done:
            env.board = prev_state
            return action1, -1  # Jeśli gra się skończyła po pierwszym ruchu
        ...
        if not possible_fields_2:
            env.board = prev_state
            return action1, -1  # Brak drugiego ruchu
        ...
        with T.no_grad():
            q_values_2 = eval_net.forward(new_state_tensor, move_dice_pair)
            q_values_2 = q_values_2 + mask2  # Ustawiamy -inf dla niedozwolonych akcji
    ...
    def learn(self, player):
        ...
        loss = (loss_1 + loss_2) / 2  # Średnia z dwóch ruchów
        loss.backward()
    ...
    def update_target_network(self, player):
        ...
        t = 0.01  # Stała określająca jak szybko kopiujemy wagi
        if player == 'white':
    ...
    def step(self, action, player, movement, save_file=False):
        ...
        if player == 'white':
            if self.count_positive_fields() == 0:
                return True, total_reward + 1, True, 'white'  # Nagroda za wygraną

        elif player == 'black':
            if self.count_negative_fields() == 0:
                return True, total_reward + 1, True, 'black'  # Nagroda za wygraną

        # Dodaj stałą nagrodę za poprawny ruch, aby zachęcać do działania

        total_reward += 0.01 * (prev_pips - env.calculate_pips(player))
        if save_file:
...
def agent_vs_agent(env, agent_white, agent_black, num_episodes=100):
    ...
    for episode in range(1, num_episodes + 1):
        ...
        while not done:
            ...
            current_agent = agent_white if current_player == 'white' else agent_black
            while not success1 and not success2:
                action_idx1 = current_agent.choose_action(env.get_state(), player=current_player,
                                                          forbidden_actions=forbidden_actions, trained=True)
                if action_idx1 == -1:
                    break
                action = (action_idx1, dice_roll[0])
                success1, reward, done, winner = env.step(action, player=current_player, movement=False,
                                                          save_file=True)
                if done:
                    winner = current_player
                    break
                if not success1:
                    forbidden_actions.add(action_idx1)

                action_idx2 = current_agent.choose_action(env.get_state(), player=current_player,
                                                          forbidden_actions=forbidden_actions, trained=True)
                if action_idx2 == -1:
                    break
                action = (action_idx2, dice_roll[0])
                success1, reward, done, winner = env.step(action, player=current_player, movement=False,
                                                          save_file=True)
                if not success2:
                    forbidden_actions.add(action_idx1)
...
def play_game(agent):
    ...
    while not done:
        ...
        if player == 'white':
            ...
            print(env.board)
            while not success:
                from_pos = int(input(f'Kostka: {dice_roll[0]}\nWpisz pole dla 1 ruchu:'))
                success = env.move(from_pos, dice_roll[0], 'white')
                if not success:
            ...
            print(env.board)
            while not success:
                from_pos = int(input(f'Kostka: {dice_roll[1]}\nWpisz pole dla 2 ruchu:'))
                success = env.move(from_pos, dice_roll[1], 'white')
                if not success:
                    print('Ruch niedozwolony')
            if env.count_positive_fields() == 0:
                done = True
            ...
            while not success1 and not success2:
                ...
                success1, reward, done, winner1 = env.step(action, player=player, movement=False)
                if not success1:
                    forbidden_moves1.add(from_pos)
                if done:
                    winner = player
                ...
                success2, reward, done, winner1 = env.step(action, player=player, movement=False)
                if not success2:
                    # reward -= 0.5
                    forbidden_moves2.add(from_pos)
                if winner1:
...
env = BackgammonEnv()
agent_white = Agent(gamma, epsilon, lr, input_size, batch_size, n_actions, eps_decay=1e-4)
agent_black = Agent(gamma, epsilon, lr, input_size, batch_size, n_actions, eps_decay=1e-4)
