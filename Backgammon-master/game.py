...
class Agent:
    def __init__(self, gamma, epsilon, lr, input_size, batch_size, n_actions, max_mem=10000, eps_end=0.01,
                 eps_decay=1e-3, exploration_type='epsilon_greedy', temp=1.0):
...
class BackgammonEnv:
    ...
    def step(self, action, player, movement, save_file=False):
        ...
        state = self.board
        # if from_pos == 0 and env.bar[player] > 0:
        #     dice_roll -= 1
        ...
        if (player == 'white' and to_pos >= 25) or (player == 'black' and to_pos <= 0):
            total_reward += 10  # Nagroda za wyjście pionka
        ...
        if 0 <= to_pos <= 23 and abs(state[to_pos]) == 1 and (
            ...
            total_reward += 5  # Nagroda za bicie pionka
        ...
        if player == 'white':
            if self.count_positive_fields() == 0:
                ...
                total_reward -= 10  # 🔹 Kara dla przegranego przeciwnika
        elif player == 'black':
    ...
    def move(self, from_pos, dice_roll, player,
             agent=Agent(gamma=0.99, epsilon=1, lr=0.0001, input_size=25, batch_size=16, n_actions=24)):
        ...
        while not done:
            for _ in range(2):  # Każdy gracz wykonuje ruch
                ...
                for die in dice_roll:
                    ...
                    state = env.get_state()
                    while not success:
                        action_idx = current_agent.choose_action(env.get_state(), die, player=current_player,
                        ...
                        if not success:
                            ...
                            if current_player == 'white':
                                transitions_white.append((state, action_idx, reward, env.board, die, done))
                                total_reward_white += reward
                            else:
                                transitions_black.append((state, action_idx, reward, env.board, die, done))
            ...
            for dice in dice_roll:
                ...
                while not success:
                    ...
                    if action_idx == -1:
                        break
                    action = (action_idx, dice)
                    success, reward, done, winner = env.step(action, player=current_player, movement=False, save_file=True)
                    if not success:
                        forbidden_actions.add(action_idx)
