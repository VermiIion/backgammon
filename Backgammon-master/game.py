...
class DQN(nn.Module):
    ...
    #     if move_dice_pairs.dim() == 1:
    #         move_dice_pairs = move_dice_pairs.unsqueeze(0)
    #
    #     x = T.cat((state, move_dice_pairs), dim=1)
    #     x = F.relu(self.fc1(x))
    #     x = F.relu(self.fc2(x))
    #     x = self.fc3(x)
    #     return x
...
class Agent:
    ...
    #     eval_net.optimizer.zero_grad()
    #
    #     max_mem = min(self.mem_pointer, len(self.state_memory))
    #     batch_indices = np.random.choice(max_mem, self.batch_size, replace=False)
    #
...
# Obliczenie różnicy nagród (biały - czarny)
