...
class Agent:
    ...
    def get_possible_fields(self, board, player):
        ...
        if player == 'white':
            ...
            if env.bar['white'] > 0:
                possible_moves = [-1]  # [i for i in range(24) if board[i] >= 0]  # Pola, na których nie ma przeciwnika
            else:
            ...
            if env.bar['black'] > 0:
                possible_moves = [24]  # [i for i in range(24) if board[i] <= 0]  # Pola, na których nie ma przeciwnika
            else:
                # Zwróć pola, z których gracz może wykonać ruch, jeśli nie ma pionków na barze
