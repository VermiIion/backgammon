...
def play_random_vs_heuristic(env, num_episodes):
    ...
    for episode in range(num_episodes):
        ...
        current_player = random.choice(['white', 'black'])  # Losowo wybiera, kto zaczyna
        winner = ''
        ...
        while not done:
            ...
            for dice in dice_roll:
                ...
                if current_player == 'white':
                    ...
                    if possible_moves:
                        action_idx = random.choice(possible_moves)
                        action = (action_idx, dice)
        ...
        if player == 'white':
            ...
            while not success:
                if env.bar['white'] > 0:
                    ...
                    # Zwróć pola, z których gracz może wykonać ruch, jeśli nie ma pionków na barze
                    possible_moves = [i for i, value in enumerate(env.board) if value > 0]  # Pionki białe
                print(f'Pola do ruchu:{possible_moves}')
            ...
            if env.bar['black'] > 0:
                ...
                # Zwróć pola, z których gracz może wykonać ruch, jeśli nie ma pionków na barze
                possible_moves = [i for i, value in enumerate(env.board) if value < 0]  # Pionki białe
            print(f'Bot: Pola do ruchu:{possible_moves}')
            success1 = False
            success2 = False
            forbidden_moves1 = set()
            forbidden_moves2 = set()
            ...
            while not success1 and not success2:
                ...
                if done:
                    ...
                    break
                if success1:
                    print(f'Bot ruch 1[skąd, kostka]: {action_idx1, dice_roll[0]}')
                if action_idx2 == -1:
                    break
                from_pos = action_idx2
                action = (from_pos, dice_roll[1])
                ...
                if winner1:
                    winner = player
                if success2:
                    print(f'Bot ruch 2[skąd, kostka]: {action_idx2, dice_roll[1]}')
